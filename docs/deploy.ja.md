...
`beacon/docs/deploy-cti-platform.ja.md` を使う。この runbook は SAGE を
`sage-api`（読み取り専用 Cloud Run service）と `sage-etl`（単一 writer Cloud Run
Job）に分けたまま、`cti-console` に `sage-api` URL と共有 GCS storage を配線する。

`SAGE_DB=spanner` などの非標準 SAGE topology や custom IAM/networking が必要な場合は、
この standalone SAGE deploy guide を使う。
