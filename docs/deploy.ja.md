...
`sage-etl`（単一 writer Cloud Run Job）に分けて維持する。`cti-console` は BEACON
リポジトリからデプロイする。console は Threats と STIX 抽出で `sage-api` を呼び、
`sage-etl` だけが共有 storage の `db/sage.db` を更新する。console の command は
BEACON deployment guide を参照する。
