...
```sh
# 全テーブルを DROP して再作成 — クリーンな状態が必要な場合のみ使用
make init-schema
```
...
- Spanner の既存データを確認: `SELECT COUNT(*) FROM ThreatActor`

### `OTEL_SDK_DISABLED` メトリクスエクスポートエラー

メトリクスバックエンドのない環境での Spanner クライアント OpenTelemetry エクスポートエラーを抑制するには、`.env` に `OTEL_SDK_DISABLED=true` を設定する。

### スキーマ初期化時の `ALREADY_EXISTS` エラー

スキーマが既に初期化されている。`make init-schema` はクリーンな状態が必要な場合のみ実行する — 全テーブルが DROP される。
