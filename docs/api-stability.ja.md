...
`src/sage/spanner/migrations/` ディレクトリにはテーブル変更ごとにバージョン管理された前進のみのマイグレーション SQL ファイルが格納される。これらのマイグレーションファイルの適用は現時点ではオペレーターが Spanner に対して手動で SQL を実行する手順となる。`sage init-schema` は引数を取らず、`schema/spanner_ddl.sql` のベーススキーマのみを適用する。
