...
**4.0.0 破壊的変更の記録（2026-06-12）**: 既定のデータベースバックエンドが
Spanner から SQLite に変更された（`SAGE_DB` 環境変数、既定値 `sqlite`;
Spanner は `SAGE_DB=spanner` で任意のバックエンドとして温存）。
`GCP_PROJECT_ID`、`SPANNER_INSTANCE`、`SPANNER_DB`、`SAGE_ETL_INPUT_BUCKET`
は `SAGE_DB=spanner` の場合のみ必須。REST API サーフェスと入力ファイル
コントラクト（STIX バンドル / `assets.json` / PIR JSON）は不変。Spanner へ
戻す手順は CHANGELOG 4.0.0 を参照。
...
`src/sage/spanner/migrations/` ディレクトリにはテーブル変更ごとにバージョン管理された前進のみのマイグレーション SQL ファイルが格納される。これらのマイグレーションファイルの適用は現時点ではオペレーターが Spanner に対して手動で SQL を実行する手順となる。`sage init-schema` は引数を取らず、設定されたバックエンドのベーススキーマ（`SAGE_DB=spanner` の場合は `schema/spanner_ddl.sql`、それ以外は `schema/sqlite_ddl.sql`）を適用する。

4.0.0 以降、同じ Committed サーフェス（テーブルの存在、カラム名、カラムの NULL 可否、主キー、デフォルト値）は `schema/sqlite_ddl.sql` にも適用される。同ファイルは文書化された型マッピング（`TIMESTAMP` → TEXT ISO 8601 UTC、`ARRAY<STRING>` → TEXT JSON 配列、`INT64` → INTEGER、`FLOAT64` → REAL、`STRING(n)` → TEXT）のもとで全 36 テーブルをミラーする。上記の追加的 / 破壊的ルールは両方の DDL ファイルに適用される。
...
| サブコマンド | 置き換え対象 | 目的 |
|---|---|---|
| `sage init-schema` | `src/sage/cli/init_schema.py` | 設定されたバックエンド（`SAGE_DB`）の DDL を適用 + インデックスを作成 |
| `sage load-assets` | `src/sage/cli/load_assets.py` | BEACON assets.json をデータベースにロード |
...
| 環境変数 | デフォルト | 目的 |
|---|---|---|
| `SAGE_DB` | `sqlite` | データベースバックエンドセレクター: `sqlite` または `spanner`（4.0.0）|
| `GCP_PROJECT_ID` | （`SAGE_DB=spanner` 時必須）| GCP プロジェクト ID |
| `SPANNER_INSTANCE` | （`SAGE_DB=spanner` 時必須）| Spanner インスタンス ID |
| `SPANNER_DB` | （`SAGE_DB=spanner` 時必須）| Spanner データベース ID |
| `SAGE_ETL_INPUT_BUCKET` | （`SAGE_DB=spanner` 時必須）| GCS ランディングゾーンバケット |
...
- **マイグレーションスクリプトの命名規則** — `src/sage/spanner/migrations/` 内のもの。マイグレーションの順序と内容は Committed（オペレーターは Spanner に対して手動でマイグレーション SQL を適用する）だが、ファイル命名は内部的。
...
- **Google Cloud Spanner Graph（Preview/GA）**: 任意のストレージレイヤーとして（`SAGE_DB=spanner`; 4.0.0 以降の既定バックエンドは Python 標準ライブラリの SQLite）。Spanner Graph DDL 構文の安定性はアップストリームが制御する。
