...
| パッケージ | 不採用理由 |
|-----------|---------|
| `neo4j` | SAGE のグラフ探索はメモリ上のエッジを走査する Python BFS（`src/sage/analysis/similarity.py`）で実装しており、CTI グラフ規模（数万〜数十万行）では専用グラフ DB を必要としない。Neo4j 導入はインフラ複雑度とコストを増加させる。規模拡大で BFS が不十分と判明した場合は再検討。|
