...
**正本**: 本ドキュメントは SAGE リポジトリ（`sage/docs/ir-feedback-flow.md`）に
置かれている。BEACON および TRACE は各 README からこのファイルを参照する。

本ドキュメントは、エンドツーエンドの「IR 観測 → SAGE 登録 → BEACON 再スコアリング →
TRACE 検索」ループを説明する。OpenCTI リレー経由のインシデント取り込みパスを
...
| 直接 | `--no-api` | エアギャップ環境 — HTTP をバイパスしてデータベースに直接書き込む |
...
`BEACON_IR_LOOKBACK_DAYS` 環境変数（デフォルト 365）。アクターごとの設定はなく、グローバルな単一設定。
...
`trace search-iocs --ioc <value> [--type <t>] [--tlp-max <level>]` は
...
| 監査証跡 | OpenCTI ログ | SAGE structlog + データベース監査 |
...
# エアギャップ（HTTP をバイパスしてデータベースに直接書き込む）:
uv run sage incident-register \
